...
// Gemini proposes where canonical concepts are taught in the reference video.
// The server remains authoritative and must validate every returned timestamp.
// ─────────────────────────────────────────────────────────────────────────────

const VIDEO_MAPPING_SCHEMA = {
  type: SchemaType.OBJECT,
  properties: {
    mappings: {
      type: SchemaType.ARRAY,
      description: 'Proposed video segments for canonical concepts',
      items: {
        type: SchemaType.OBJECT,
        properties: {
          levelOrder: {
            type: SchemaType.INTEGER,
            description: 'Canonical level order (1-based)',
          },
          conceptOrder: {
            type: SchemaType.INTEGER,
            description: 'Canonical concept order within the level (1-based)',
          },
          startSeconds: {
...
const VIDEO_MAPPING_SYSTEM_PROMPT = `You are an expert computer science educator helping map a canonical DSA curriculum to a reference teaching video.

Your task is to identify the timestamp range where each supplied canonical concept is actually taught in the supplied transcript.

Rules:
- The canonical curriculum is the source of truth for concept identity.
- The transcript is the source of truth for what the video actually teaches.
- Only map a concept when the transcript provides meaningful instructional evidence for that concept.
- Do NOT invent concepts.
- Do NOT invent timestamps.
- Do NOT map a concept merely because the transcript mentions its name briefly.
- Prefer the smallest continuous range that contains the actual explanation of the concept.
- A concept may be left unmapped when there is insufficient evidence.
- Do NOT map one concept to another concept's explanation.
- levelOrder and conceptOrder must exactly match the supplied canonical concepts.
- startSeconds and endSeconds must correspond to the supplied transcript timestamps.
- Return only mappings supported by the transcript.
- Do not return explanations, confidence scores, or additional fields.`;



/**
 * Maps canonical course concepts to timestamp ranges in a transcript batch.
 *
 * This function intentionally does NOT decide whether the returned mappings
 * are safe to persist. Callers must validate the timestamps against the
 * transcript and canonical curriculum before saving them.
 *
 * @param {Array} concepts
 * @param {Array} transcriptBlocks
 * @returns {Promise<{ mappings: Array }>}
 */
export async function mapVideoConcepts(concepts, transcriptBlocks) {
  if (!Array.isArray(concepts) || concepts.length === 0) {
    throw new Error('Video mapping requires at least one canonical concept.');
  }

  if (!Array.isArray(transcriptBlocks) || transcriptBlocks.length === 0) {
    throw new Error('Video mapping requires transcript blocks.');
  }
  ...
  const model = client.getGenerativeModel({
    model: 'gemini-2.5-flash',
    systemInstruction: VIDEO_MAPPING_SYSTEM_PROMPT,
    generationConfig: {
      responseMimeType: 'application/json',
      responseSchema: VIDEO_MAPPING_SCHEMA,
    },
  });
  ...
  const conceptsText = concepts
    .map(
      (concept) =>
        `Level ${concept.levelOrder}, Concept ${concept.conceptOrder}: ${concept.title}
Description: ${concept.description || ''}
Objectives: ${concept.learningObjectives?.join(', ') || ''}`
    )
    .join('\n\n');
  ...
  const transcriptText = transcriptBlocks
    .map(
      (block) =>
        `[${block.startSeconds.toFixed(3)} - ${block.endSeconds.toFixed(3)}] ${block.text}`
    )
    .join('\n');

  const userPrompt = `Canonical concepts:
---
${conceptsText}
---

Timestamped reference-video transcript:
---
${transcriptText}
---

Map each canonical concept to the smallest continuous timestamp range that contains meaningful teaching of that concept.

If a concept is not meaningfully taught in this transcript, omit it from the mappings array.

Return only the requested structured mappings.`;
  ...
  try {
    ...
    console.error('[gemini] video mapping generateContent failed:', err.message);
    throw new Error('Gemini API call failed during video mapping.');
  }
  ...
  try {
    ...
    console.error(
      '[gemini] Video mapping response was not valid JSON:',
      text?.slice(0, 200)
    );
    throw new Error('Gemini returned malformed video mapping output.');
  }

  validateVideoMappingOutput(parsed, concepts);

  return parsed;
}


function validateVideoMappingOutput(data, concepts) {
  if (!data || typeof data !== 'object' || !Array.isArray(data.mappings)) {
    throw new Error('Video mapping output must contain a mappings array.');
  }

  const validConcepts = new Set(
    concepts.map(
      (concept) => `${concept.levelOrder}:${concept.conceptOrder}`
    )
  );
  ...
  for (const [index, mapping] of data.mappings.entries()) {
    if (!Number.isInteger(mapping.levelOrder) || mapping.levelOrder < 1) {
      throw new Error(
        `Video mapping ${index + 1} has an invalid levelOrder.`
      );
    }

    if (!Number.isInteger(mapping.conceptOrder) || mapping.conceptOrder < 1) {
      throw new Error(
        `Video mapping ${index + 1} has an invalid conceptOrder.`
      );
    }

    const conceptKey =
      `${mapping.levelOrder}:${mapping.conceptOrder}`;

    if (!validConcepts.has(conceptKey)) {
      throw new Error(
        `Video mapping ${index + 1} references a non-canonical concept.`
      );
    }

    if (seenConcepts.has(conceptKey)) {
      throw new Error(
        `Duplicate video mapping for ${conceptKey}.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Video mapping ${index + 1} has an invalid startSeconds.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Video mapping ${index + 1} has an invalid endSeconds.`
      );
    }
  }
}
